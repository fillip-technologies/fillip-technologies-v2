...
const INDUSTRY_ICONS = [GraduationCap, HeartPulse, Building2, Plane, Utensils, ShoppingBag, Sparkles, Store, BriefcaseBusiness] as const;
    ...
    <main ref={pageRef} className="overflow-hidden bg-white text-heading">
        ...
        <div className="mx-auto grid max-w-7xl items-center gap-16 lg:grid-cols-[1.02fr_.98fr]">
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, ease }}>
            ...
            <div className="mt-9 flex flex-wrap gap-3">
              <Link href="/contact" className="group inline-flex items-center gap-2 rounded-full bg-heading px-6 py-3.5 text-sm font-semibold text-white shadow-[0_18px_50px_rgba(15,23,42,.18)] transition hover:-translate-y-0.5 hover:bg-navy">
                {hero.primaryCtaLabel} <ArrowRight className="size-4 transition-transform group-hover:translate-x-1" />
              </Link>
              <a href="#workflow" className="inline-flex items-center gap-2 rounded-full border border-border bg-white/70 px-6 py-3.5 text-sm font-semibold text-heading shadow-sm backdrop-blur transition hover:-translate-y-0.5 hover:border-primary/25 hover:bg-white">
                {hero.secondaryCtaLabel} <ArrowDown className="size-4" />
              </a>
            </div>
      ...
      <section className="px-6 py-24 lg:py-32">
        <div className="mx-auto max-w-7xl">
          <motion.div {...reveal} className="grid gap-8 lg:grid-cols-[.75fr_1.25fr] lg:items-end">
            <div><p className="text-xs font-bold uppercase tracking-[0.2em] text-primary">{problem.eyebrow}</p><h2 className="mt-4 text-4xl font-bold tracking-[-0.045em] md:text-6xl">{problem.heading}</h2></div>
            <p className="max-w-2xl text-lg leading-8 text-body lg:justify-self-end">{problem.description}</p>
          </motion.div>
      ...
      <section id="workflow" className="relative border-y border-border bg-surface px-6 py-24 text-heading lg:py-32">
        ...
        <div className="relative mx-auto max-w-7xl">
          <motion.div {...reveal} className="max-w-3xl"><p className="text-xs font-bold uppercase tracking-[0.2em] text-primary">{solution.eyebrow}</p><h2 className="mt-4 text-4xl font-bold tracking-[-0.045em] md:text-6xl">{solution.heading}</h2><p className="mt-5 max-w-2xl text-lg leading-8 text-body">{solution.description}</p></motion.div>
      ...
      <section className="border-y border-border bg-surface px-6 py-24 lg:py-32">
        <div className="mx-auto max-w-7xl"><motion.div {...reveal} className="max-w-3xl"><p className="text-xs font-bold uppercase tracking-[0.2em] text-primary">{industries.eyebrow}</p><h2 className="mt-4 text-4xl font-bold tracking-[-0.045em] md:text-6xl">{industries.heading}</h2></motion.div><div className="wa-reveal mt-16 grid border-l border-t border-border sm:grid-cols-2 lg:grid-cols-3">{industries.industries.map(({ label }, index) => { const Icon = INDUSTRY_ICONS[index % INDUSTRY_ICONS.length]; return <motion.div whileHover={{ backgroundColor: "#ffffff" }} key={label || index} className="group flex min-h-36 items-end justify-between border-b border-r border-border p-6 transition"><div><span className="text-xs text-body">0{index + 1}</span><div className="mt-8 text-xl font-bold">{label}</div></div><Icon className="size-6 text-primary transition-transform group-hover:-translate-y-1 group-hover:scale-110" /></motion.div>; })}</div></div>
      </section>
      ...
      <section className="bg-[#071a23] px-6 py-24 text-white lg:py-32">
        <div className="mx-auto grid max-w-7xl gap-16 lg:grid-cols-[.8fr_1.2fr]">
          <motion.div {...reveal}><p className="text-xs font-bold uppercase tracking-[0.2em] text-emerald-300">{whyus.eyebrow}</p><h2 className="mt-4 text-4xl font-bold tracking-[-0.045em] md:text-6xl">{whyus.heading}</h2><p className="mt-6 text-lg leading-8 text-white/55">{whyus.description}</p></motion.div>
          <div className="wa-reveal divide-y divide-white/10 border-y border-white/10">{whyus.items.map(({ number, title, copy }) => <motion.div whileHover={{ x: 5 }} key={title} className="grid gap-3 py-5 sm:grid-cols-[48px_1fr_1.35fr] sm:items-center"><span className="text-xs text-emerald-300">{number}</span><span className="font-semibold">{title}</span><span className="text-sm leading-6 text-white/45">{copy}</span></motion.div>)}</div>
        </div>
      </section>

      {/* Results Section */}
      <section className="px-6 py-24 lg:py-32">
        <div className="mx-auto max-w-7xl"><motion.div {...reveal} className="flex flex-col justify-between gap-6 md:flex-row md:items-end"><div><p className="text-xs font-bold uppercase tracking-[0.2em] text-primary">{results.eyebrow}</p><h2 className="mt-4 text-4xl font-bold tracking-[-0.045em] md:text-6xl">{results.heading}</h2></div><p className="max-w-md text-body">{results.description}</p></motion.div><div className="wa-reveal mt-16 grid border-l border-t border-border sm:grid-cols-2 lg:grid-cols-4">{results.metrics.map(({ value, suffix, label }) => <div key={label} className="border-b border-r border-border p-7 lg:p-9"><div data-count={value} data-suffix={suffix} className="text-5xl font-bold tracking-[-0.05em] text-primary md:text-6xl">0{suffix}</div><div className="mt-6 text-sm font-semibold">{label}</div><div className="mt-2 h-1 w-10 rounded-full bg-emerald-400" /></div>)}</div></div>
      </section>
