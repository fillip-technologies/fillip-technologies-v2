...
export default function WhyFillipSection({
    data,
}: {
    data: AIAutomationContent["why"];
}) {
    return (
        <section className="py-24 lg:py-32">
            <div className="mx-auto grid max-w-7xl gap-14 px-6 lg:grid-cols-12">
                ...
                <motion.div
                    {...reveal}
                    className="relative border-l border-border pl-7 sm:pl-10 lg:col-span-7"
                >
                    {data.items.map((item, i) => (
                        <div
                            key={item.title}
                            className="relative border-b border-border py-7 first:pt-0 last:border-0"
                        >
