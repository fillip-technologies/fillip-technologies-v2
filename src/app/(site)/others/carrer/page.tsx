...
export default async function CareerPage() {
  ...
  return (
    <main className="bg-white text-slate-950">
      <section className="relative overflow-hidden bg-gradient-to-b from-[#edf5ff] via-[#f8fbff] to-white pb-24 pt-40 text-slate-950 lg:pb-32 lg:pt-48">
        <div className="absolute inset-0 opacity-[0.05]" style={{ backgroundImage: "linear-gradient(to right,#0242a2 1px,transparent 1px),linear-gradient(to bottom,#0242a2 1px,transparent 1px)", backgroundSize: "76px 76px" }} />
        <div className="absolute left-1/2 top-0 h-[500px] w-[850px] -translate-x-1/2 rounded-full bg-primary/15 blur-[150px]" />
        <div className="relative mx-auto max-w-7xl px-6 lg:px-10">
          <p className="text-sm font-semibold uppercase tracking-[0.25em] text-accent">{hero.eyebrow as string}</p>
          <div className="mt-7 grid items-end gap-10 lg:grid-cols-[1fr_0.65fr]"><h1 className="max-w-4xl text-6xl font-bold leading-[0.95] tracking-[-0.05em] sm:text-7xl lg:text-8xl">{hero.headingLead as string} <span className="text-primary">{hero.headingHighlight as string}</span></h1><div><p className="max-w-lg text-lg leading-8 text-slate-600">{hero.subheadline as string}</p><Link href="#open-roles" className="mt-8 inline-flex items-center gap-2 rounded-full bg-primary px-6 py-3 text-sm font-semibold text-white">{hero.ctaLabel as string} <ArrowDown className="size-4" /></Link></div></div>
        </div>
      </section>

      <section className="py-24 lg:py-32"><div className="mx-auto max-w-7xl px-6 lg:px-10"><div className="grid gap-12 lg:grid-cols-2 lg:items-center"><div><span className="text-xs font-semibold uppercase tracking-[0.22em] text-primary">{team.eyebrow as string}</span><h2 className="mt-5 text-5xl font-bold leading-none tracking-tight lg:text-6xl">{team.heading as string}</h2><p className="mt-7 max-w-xl text-lg leading-8 text-slate-600">{team.description as string}</p><div className="mt-10 grid grid-cols-3 gap-5 border-t border-slate-200 pt-8"><div><strong className="text-3xl text-primary">{team.stat1Value as string}</strong><p className="mt-1 text-sm text-slate-500">{team.stat1Label as string}</p></div><div><strong className="text-3xl text-primary">{team.stat2Value as string}</strong><p className="mt-1 text-sm text-slate-500">{team.stat2Label as string}</p></div><div><strong className="text-3xl text-primary">{team.stat3Value as string}</strong><p className="mt-1 text-sm text-slate-500">{team.stat3Label as string}</p></div></div></div><div className="grid grid-cols-2 gap-5"><div className="relative h-[430px] overflow-hidden rounded-[32px] bg-white"><Image src={team.image1 as string} alt="Fillip Technologies team member" fill sizes="(max-width: 1024px) 50vw, 320px" className="object-cover object-center" /></div><div className="relative mt-14 h-[430px] overflow-hidden rounded-[32px] bg-white"><Image src={team.image2 as string} alt="Fillip Technologies team member" fill sizes="(max-width: 1024px) 50vw, 320px" className="object-cover object-center" /></div></div></div></div></section>
