...
import { setCaseStudyPublishedAction } from "@/server/content/casestudy-actions";
...
export default function StatusBar({ slug, published }: { slug: string; published: boolean }) {
  ...
  const toggle = () => {
    ...
    startTransition(async () => {
      const res = await setCaseStudyPublishedAction(slug, !published);
  ...
  return (
    <div className="mb-6 flex flex-wrap items-center gap-3 rounded-lg border border-border bg-card/40 px-4 py-3">
      ...
      <a
        href={`/case-studies/${slug}/preview`}
        target="_blank"
