...
/**
 * Edits the mega-menu sub-links shown under a "What We Do" category header.
 * `onSave` is a server action bound to the category slug. The URL is optional —
 * a link with none renders as a non-clickable label in the menu.
 */
export default function MenuLinksEditor({
  onSave,
  initial,
}: {
  onSave: (items: MegaMenuItem[]) => Promise<SaveState>;
  initial: MegaMenuItem[];
}) {
  const [items, setItems] = useState<MegaMenuItem[]>(() =>
    initial.map((i) => ({ label: i.label ?? "", href: i.href ?? "" }))
  );
  const [state, setState] = useState<SaveState | null>(null);
  const [pending, startTransition] = useTransition();

  const setField = (idx: number, key: "label" | "href", v: string) =>
  ...
  const save = () => {
    startTransition(async () => setState(await onSave(items)));
  };
  ...
  return (
    <div className="space-y-4">
        ...
        {items.map((item, idx) => (
          <div key={idx} className="rounded-lg border border-border bg-card/40 p-4">
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              ...
              <div>
                <label className="mb-1 block text-xs text-body">URL (optional)</label>
                <input
                  value={item.href ?? ""}
      ...
      <div className="flex items-center gap-3 border-t border-border pt-4">
        ...
        >
          {pending ? "Saving…" : "Save menu links"}
        </button>
        {state?.message ? (
